...
import {MessageElemExt} from '../types/MessageElemExt'
import "@lottiefiles/lottie-player/dist/tgs-player";

export default function MessageElement({elem}: { elem: MessageElemExt }) {
  switch (elem.type) {
    case 'text':
    case 'face':
    case 'sface':
    ...
    case 'flash':
      ...
      />
    case 'video-loop':
      return <video src={elem.url} autoPlay muted loop width={200}/>
    case 'tgs':
      return <tgs-player autoplay loop mode="normal" src={elem.url} style={{width: 200, height: 200}}/>
    case 'video':
      return <div>[视频]</div>
    case 'record':
      return <div>[语音]</div>
